...
<div>
	...
	<div style="position:relative; left: 25px; top: 0px; z-index: 0; width: 700px">
		...
		<div style="position: relative; left: 0px">
			...
			<h3>Edit The Plants You Supply</h3></center>
		</div><br><br>
		...
		<div>
										...
										// banner rotating section
											...
											while($row = mysqli_fetch_array($result)) {
																...
																if ($xxx == 0) {
																	...
																	if (($names[1] == $row['p_id']) || ($names[2] == $row['p_id']) || ($names[4] == $row['p_id']) || ($names[5] == $row['p_id']) || ($names[7] == $row['p_id']) || ($names[8] == $row['p_id']) || ($names[10] == $row['p_id']) || ($names[11] == $row['p_id']) || ($names[13] == $row['p_id']) || ($names[14] == $row['p_id']) || ($names[16] == $row['p_id']) || ($names[17] == $row['p_id']) || ($names[19] == $row['p_id']) || ($names[20] == $row['p_id']) || ($names[22] == $row['p_id']) || ($names[23] == $row['p_id'])) {$chi2 = 'checked';}else{$chi2='&nbsp';}																	
																	...
																	if (($names[3] == $row['p_id']) || ($names[6] == $row['p_id']) || ($names[9] == $row['p_id']) || ($names[12] == $row['p_id']) || ($names[15] == $row['p_id']) || ($names[18] == $row['p_id']) || ($names[21] == $row['p_id']) || ($names[24] == $row['p_id'])) {$chi3 = 'checked';}else{$chi3='&nbsp';}																	
			...
			?>				
                       ...
                       <input type="submit" value="Submit Plant Updates" style=" background-color: darkgoldenrod; box-shadow: 5px 5px 5px #888888; padding: 3px; font-size: 16px; cursor: pointer" />
			</form>	
