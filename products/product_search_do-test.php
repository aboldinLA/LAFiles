...
$proceed = true;
    ...
    if(($keyword=="" || !isset($keyword)) && ($localtype=="" || !isset($localtype))){
        ...
        echo "<div id='pageBody'>
            <center>
        	<table width='100%' border='0' align='center'><br>Keyword Empty.</table>
            </center>
        </div>";
        $proceed = false;
    }
...
if($searchtype == 'company' && $proceed == true){
    ...
    if(!$results){
        echo "<div id='pageBody'>
            <center>
        	<table width='100%' border='0' align='center'><br>Click Here to Modify Search.</table>
            </center>
        </div>";
    }
    else
...
if($searchtype == 'photo' && $proceed == true){
    ...
    if(!$results) {
        echo "<div id='pageBody'>
        <center>
        	<table width='100%' border='0' align='center'><br>No Matches Found. Please Try Again.</table>
            </center>
        </div>";
    }
    else
...
function company_results_html($results){
    ...
    foreach($results as $r){
    	...
    	if($counter % 2 != 0)
    	   $html .= "    <TD width=30>&nbsp;</TD>";
    	$html .= "	<td width=226 bgcolor=#EFEFEF>";
	    ...
	    $html .="	<br>";
	    /*
	    $html .= $r['c_phone_area_code'] ? $r['c_phone_area_code'] : $r['phone_area_code'];
	    $html .= "-";
	    $html .= $r['c_phone_number'] ? $r['c_phone_number'] : $r['phone_number'];
	    */
	    $html .= $r['phone_area_code'];
	    $html .= "-";
	    $html .= $r['phone_number'];
	    $html .= "</td>";
    	$html .= "    <td bgcolor=#EFEFEF align=center>".get_typecode($r[vendor_type_id])."</td>";
    	if($counter % 2 != 0)
    	   $html .= "</tr>";
...
function photo_results_html($results){
    ...
    foreach($results as $r){
        ...
        if($counter % 3 == 0)
            $html .= "<tr>";
        $html .= "<td align=center><div style='overflow: hidden; width: 164px;'>";
        if($r['status'] >= 4) $html .= "<a href='listing.php?id=$r[vendor_id]'>";
        $html .= "<img src='images/$r[photo]' border='0'width='164px'></div><br />$r[company_name]";
        if($r['status'] >= 4) $html .= "</a>";
        $html .= "<br /><br /></td>";
...
function search_photo($next){
    global $searchby, $vst, $find, $state, $areacodes;
        ...
        $query = "select vp.*,v.company_name,v.status,v.edit_date,vp.photo_time 
                  ...
                  left join contacts as c on (c.ref_id=v.id AND c.ref_type='ven' AND c.code='company')
                  inner join contacts AS co ON (co.ref_id = v.id AND co.ref_type='ven' AND co.code='company')
                  where v.active=1 and c.active=1 and v.status>0 ";
        ...
        if($searchby == "ac") {
	        	...
	        	if($areacodes != ""){
	        		...
	        		foreach ($ary_areacodes as $val){
	        			$ary_sql_area_codes[] = "(c.phone_area_code = '$val')"; 
	        		}
	        		
	        		
	        		
	           		$query .= "AND ( ".join(" OR ",$ary_sql_area_codes)." ) ";// OR c.other_1 = $numb) ";
	        	}
        	
//            $query .= "AND (c.phone_area_code = '$areacodes') ";// OR c.other_1 = $numb) ";
        } else if($searchby == "st") {
	    ...
	    // if($searchby != "ac") { 
		// 	if($_GET["next"] == "34"){
		//     	$ary_sql_area_codes[] = "(c.phone_area_code = '213')"; 
		 //    	$ary_sql_area_codes[] = "(c.phone_area_code = '818')"; 
		// 		$query .= "AND ( ".join(" OR ",$ary_sql_area_codes)." ) ";// OR c.other_1 = $numb) ";
		// 	}
	    // }
 ...
 * @return array of company informations
 */
...
function search_photo_key($keyword){
    ...
    if($where != "") $where .= ' OR '; 
        $query = "select vp.*,v.company_name,v.status 
        			from vendor_product as vp
                    left join new_vendor as v on vp.vendor_id = v.id
                    left join contacts as c on (c.ref_id=v.id AND c.ref_type='ven')
                    inner join contacts AS co ON (co.ref_id = v.id AND co.ref_type='ven' AND co.code='company')
...
function find_listings_key($searchby, $extra, $vst, $find, $state, $areacodes) {
    $query = "SELECT " .
                  "v.*, c.phone_number, c.phone_area_code, c.other_1, " .
