				...
				<div style="position:absolute; left: 300px; top:275px; z-index: 0">
									...
									<?
										...
										// start for the banner add counting and getting from table
											...
											$sql = "SELECT * FROM new_vendor LEFT JOIN vendor_product ON new_vendor.id=vendor_product.vendor_id WHERE vendor_product.xlist='". $key4 ."' AND (new_vendor.status='16' OR new_vendor.status='18' OR new_vendor.status='14' OR new_vendor.status='12') AND new_vendor.state='". $skey ."' ORDER BY new_vendor.company_name ASC";
										...
										// banner rotating section
											   ...
											   while($row = mysqli_fetch_assoc($result)) {
												...
												echo "<div style='position:relative; left: 55px; top:125px; z-index: 0'>";
												}elseif($zCount == 2){
												...
												echo "<div style='position:relative; left: 55px; top:495px; z-index: 0'>";
												}elseif($zCount == 36){
												echo "<div style='position:relative; left: 55px; top:515px; z-index: 0'>";
												}elseif($zCount == 37){
												echo "<div style='position:relative; left: 55px; top:515px; z-index: 0'>";
												}
					   				...
					   				// 18 Start
										...
										// Check connection
											...
											$sql525 = "SELECT DISTINCT vendor_id FROM new_vendor LEFT JOIN vendor_product ON new_vendor.id=vendor_product.vendor_id WHERE vendor_product.xlist='". $key4 ."' AND new_vendor.status='18' AND new_vendor.state='". $skey ."' ORDER BY RAND()";
								...
								// End of SE4 sort
					   			...
					   			// 16 Start
										...
										// start produt row count from table
											...
											$sql25 = "SELECT * FROM new_vendor LEFT JOIN vendor_product ON new_vendor.id=vendor_product.vendor_id WHERE vendor_product.xlist='". $key4 ."' AND new_vendor.status='16' AND new_vendor.state='". $skey ."' ORDER BY vendor_product.vendor_id DESC";
											$result25 = $conn->query($sql25);
											...
											while($row = mysqli_fetch_array($result25)) {
												...
												if ($row['vendor_id'] != $rowNum) {
													...
													$result16 = $conn->query($sql16);
															...
															$ii = 0;
															$ss = 1;
															echo "<table style='position: relative; left: 75px; top: 25px'><tr>";
											...
											while($row = mysqli_fetch_array($result25)) {
												...
												if ($row['vendor_id'] != $rowNum) {
													...
													$result16 = $conn->query($sql16);
															...
															$ii = 0;
															$ss = 1;
															echo "<table style='position: relative; left: 75px'><tr>";
															echo '<td rowspan="3" width="250px"><img width="60%" style="height: auto" src="https://landscapearchitect.com/products/images/' . $row['logo'] . '"></td>';
														while($row = mysqli_fetch_array($result16)) {
																if ($ii == 0 ) {														
																	echo "<td align='left'><div><center><figure><a href='http://" . $row['web'] . "' target='_blank'><img style='max-height: 169px; max-width: 250px; overflow-x: hidden; overflow-y: hidden' src='https://landscapearchitect.com/products/images/" . $row['photo'] . "'></a><br><figcaption style='width:100%'>" . iconv('CP1252', 'UTF-8//TRANSLIT', (stripslashes($row['product_name']))) . "</figcaption></figure></center></div></td><td style='width: 5px'>&nbsp;</td>";
                   ...
                   echo '<center><h3 style="font-family: Helvetica, Arial; font-size: ; font-weight:400">Additional Suppliers</h3></center>';
										...
										// start for the banner add counting and getting from table
											...
											$sql2333 = "SELECT * FROM new_vendor WHERE xlist LIKE '%" . $key4 . "%' AND (status='16' OR status='18' OR status='14' OR status='12') ORDER BY company_name ASC";
										...
										// banner rotating section
											...
											while($row = mysqli_fetch_array($result2333)) {
																if ($xxx == 0 && $zzz < 50) {														
																	...
																	echo "</tr><tr><td style='line-height: 10px'>&nbsp;</td></tr><tr><td align='left'><div><a href='https://landscapearchitect.com/products/listing-a.php?number=" . $row['id'] . "' target='_blank'><img style='max-height: 135px; max-width: 200px; overflow-x: hidden; overflow-y: hidden' src='https://landscapearchitect.com/products/images/" . $row['logo'] . "'></a><br><table><tr><td style='line-height: 5px'>&nbsp;</td></tr></table></div></td><td style='width: 5px'>&nbsp;</td>";
																	$zzz++;
																	$xxx = 1;																	
