...
<div>
	...
	<div style="position:relative; left: 25px; top: 0px; z-index: 0; width: 700px">
		...
		<div style="position: relative; left: 0px">
			...
			<h3>Edit Your Lighting Brands</h3></center>
		</div><br><br>
		...
		<div>
				...
				<?
									...
									// Category Name Start
										...
										// Check connection
											...
											$sql = "SELECT * FROM `brands_ligh` WHERE `l_id` LIKE '%l%' ORDER BY name ASC";
											$result=mysqli_query($conn,$sql);
										...
										// banner rotating section
											...
											$sql22 = "SELECT * FROM `brands_ligh` WHERE `l_id` LIKE '%l%' ORDER BY name ASC";
										...
										// banner rotating section
											...
											while($row = mysqli_fetch_array($result)) {
																...
																if ($xxx == 0) {
																	...
																	if (($names[1] == $row['l_id']) || ($names[2] == $row['l_id']) || ($names[4] == $row['l_id']) || ($names[5] == $row['l_id']) || ($names[7] == $row['l_id']) || ($names[8] == $row['l_id']) || ($names[10] == $row['l_id']) || ($names[11] == $row['l_id']) || ($names[13] == $row['l_id']) || ($names[14] == $row['l_id']) || ($names[16] == $row['l_id']) || ($names[17] == $row['l_id']) || ($names[19] == $row['l_id']) || ($names[20] == $row['l_id']) || ($names[22] == $row['l_id']) || ($names[23] == $row['l_id'])) {$chi2 = 'checked';}else{$chi2='&nbsp';}																	
																	...
																	if (($names[3] == $row['l_id']) || ($names[6] == $row['l_id']) || ($names[9] == $row['l_id']) || ($names[12] == $row['l_id']) || ($names[15] == $row['l_id']) || ($names[18] == $row['l_id']) || ($names[21] == $row['l_id']) || ($names[24] == $row['l_id'])) {$chi3 = 'checked';}else{$chi3='&nbsp';}																	
			...
			?>				
                       ...
                       <input type="submit" value="Submit Lighting Updates" style=" background-color: darkgoldenrod; box-shadow: 5px 5px 5px #888888; padding: 3px; font-size: 16px; cursor: pointer" />
			</form>	
