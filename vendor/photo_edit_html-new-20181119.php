...
<div class="tab">
  ...
  <button class="tablinks" onclick="openCity(event, 'Paris')">List View</button>
	...
	<option value="https://landscapearchitect.com/vendor/index-vendor-js.php?action=edit&tview=allp" selected ><?  if (($sName == "allp") || ($sName == "")) { echo "Refine Search"; } elseif ($sName == "other") { echo $xChoice; } ?></option>
	<option value="https://landscapearchitect.com/vendor/index-vendor-js.php?action=edit&tview=allp#defpro" >All Products</option>  
					 ...
					 $sql33 = "select * from vendor_product LEFT JOIN xlist ON vendor_product.xlist = xlist.id WHERE vendor_id='" . $key2 . "' ORDER BY name ASC";
						...
						while($row = mysqli_fetch_array($result33)) { 
							...
							if ($xname != $row['name']) {
										...
										echo '<option value="https://landscapearchitect.com/vendor/index-vendor-js.php?action=edit&tview=other&xlist=' . $row['xlist'] . '#defpro" >' . $row['name'] . '</option>';
