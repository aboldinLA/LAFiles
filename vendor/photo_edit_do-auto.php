...
function get_products_list_html($company_id){
    ...
    foreach($products_list as $product){
       ...
       $html .="<br><br>$demogs2<br>".get_current_category_selected($product['id'],$company_id)."<br>";

 		$dog = $product[web];
		$html .='<br><strong>Web Link:</strong><br><a href="' . $dog . '">' .  $product[web] .  '</a>' .  '<a href="/private/vendor/index2-auto.php?action=products&intent=edit&record=' . $company_id .  '&pid=' . $product[id] .  '&pass=' . $pass . '">' . '&nbsp;&nbsp;(edit link)<br>' . '</a>' . '</td>';	
		
		
		
        $html .="<td>";
        $html .="<img src='/products/images/$product[photo]' align='right' style='box-shadow: 5px 5px 5px #888888'>";
        $html .="</td>";
        $html .="</tr>";
...
function get_current_category_selected($product_id,$company_id){
    ...
    $str .= "(<a href='photo_edit_categories-auto.php?product_id=$product_id&company_id=$company_id&pass=$pass'>edit</a>)<br>";
    if(count($product_types) == 0 || $product_types[0] == ''){
    	$str .= 'none - ' . "(<a href='photo_edit_categories.php?product_id=$product_id&company_id=$company_id&pass=$pass'>add</a>)<br>";
        return $str;
    }
...
function get_current_web_selected($product_id,$company_id){
    $query = "select web from vendor_product where id = $product_id";
    $result = query($query);
    $product_types = explode(',',$result[0]['web']);

    $str = "";
    $counter = 0;
    
    
    $str = '<br><b>Currently Selected Web Page</b>&nbsp;';
    $str .= "(<a href='photo_edit_categories-auto.php?product_id=$product_id&company_id=$company_id'>edit</a>)<br>";
    if(count($product_types) == 0 || $product_types[0] == ''){
    ...
    foreach($product_types as $category){
        if($counter!=0)
            $str .= '<br>';
        $query = "select sub_number, name from web where id = $category order by sub_number";
        $result = query($query);
        $str .= substr($result[0]['sub_number'],1);
        $str .= ' - ';
        $str .= $result[0]['name'];
