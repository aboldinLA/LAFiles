	...
	if(!strlen($error))	 {
		...
		if ($action == "edit" ||  $action == "renew" ) {
			...
			if (is_numeric($total)) { 
				...
				<meta http-equiv="refresh" content="0; URL=http://67.222.144.12/subscriptions/have_listing.php?total=<?php echo urlencode($total); ?>">  
				<?php	
				exit;
			} else {
...
<!-- Horizontal Bar End -->   
   ...
   <div style="position:absolute; left:0px; top:935px; padding:0px; width:750px; height:30px; font-family:Arial, Helvetica, sans-serif; font-size:22px; color:#C60">
    ...
    <div style="position:absolute; left:310px; top:950px; z-index:5000; padding:60px; width:299px; height:30px;font size:14px">
						<? $M->brand_sel2($brand, $protype, 'highlight'); ?>
	</div>        
        
        
    <div style="position: absolute; left: 371px; top: 1040px; padding: 0px; width: 500px; font-size:16px">
				<INPUT TYPE="checkbox" value="LAdigital" name="sal" <?=$sal?>> Also Request a Digital Version of LASN.<br />
                <p style="height:173px">&nbsp;</p>
				<br><INPUT TYPE="checkbox" value="LCdigital" name="list" <?=$list?>> Also Request a Digital Version of LC/DBM.
...
<!-- Horizontal Bar End -->     
		...
		<strong><span style="font-size:22px; color:#C60">Also We Need A Personal Identifier</span></strong><br />
                ...
                <INPUT NAME="month" SIZE="40" VALUE="<?=$DISPLAY['month']?>"  style="height:20px; font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:bold; color:#000; background-color:#CCC; padding-left:10px; box-shadow: 5px 5px 5px #888888" placeholder="First Letter of Your City of Birth"><br />           
			</div>     
