				...
				<div style="position:relative; left: 50px; top:60px; z-index: 0; padding-bottom: 50px">
									...
									<?
										...
										// Logo Sort Section
											...
											$sql = "SELECT * FROM `new_vendor` WHERE (`status` = 10 OR `status` = 12 OR `status` = 14 OR `status` = 16 OR `status` = 18) AND `xlist` LIKE '%" . $key4 . "%'  AND (vendor_type_id LIKE '6' || vendor_type_id LIKE '4') AND outlets IS NOT NULL ORDER BY `company_name` ASC";
											...
											$result = $conn->query($sql);									
												...
												while($row = mysqli_fetch_assoc($result)) {
													...
													echo $row['state'] . $row['id'] . $row['info_request'] . "&nbsp;&nbsp;";
													
													
													if (($row['state'] == $sta ) && (empty($row['info_request']))) {
													...
													} elseif (($row['state'] == $sta ) && ($row['info_request'] == 1)) {
													...
													} elseif (($row['state'] != $sta ) && ($row['outlets'] == 13610 )) {
