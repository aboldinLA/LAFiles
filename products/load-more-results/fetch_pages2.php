...
$results = mysqli_query($connecDB,"select * from vendor_product LEFT JOIN new_vendor ON new_vendor.id=vendor_product.vendor_id LEFT JOIN xlist ON xlist.id=vendor_product.xlist where (xlist.name like '%" . $key . "%'AND xlist.id > '99'OR vendor_product.description like '%" . $key . "%'OR vendor_product.product_name like '%" . $key . "%') ORDER by new_vendor.status DESC, vendor_product.photo_time DESC LIMIT $position, $item_per_page");
...
$query ="select * from vendor_product LEFT JOIN new_vendor ON new_vendor.id=vendor_product.vendor_id LEFT JOIN xlist ON xlist.id=vendor_product.xlist where (xlist.name like '%" . $key . "%'AND xlist.id > '99') ORDER by vendor_product.photo_time DESC ";
...
$query2 ="select * from banner_ups where product IN ('$x', '$q', '$z') AND picture IS NOT NULL ORDER BY RAND()";
$result2 = $db_handle->runQuery($query2);


echo "<div style='position:absolute; left:782px; top:-250px'> ";
	echo "<table width='240'>";
	
foreach($result2 as $country) {
	echo "<tr><td>";
	echo "<img src='/banner/" . $country["picture"] . "' width=240 border=0 style='margin-bottom:7px'";
	echo "</td></tr>";
}
...
<div style="position:absolute; left:-229px; top:-175px">
