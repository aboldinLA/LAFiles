...
<div>
	...
	<div style="position:relative; left: 25px; top: 0px; z-index: 0; width: 700px">
		...
		<div style="position: relative; left: 0px">
			...
			<h3>Edit Your Irrigation Brands</h3></center>
		</div><br><br>
		...
		?>
				...
				<?
									...
									// Category Name Start
										...
										// Check connection
											...
											$sql = "SELECT * FROM `brands_irri` WHERE `i_id` LIKE '%i%' ORDER BY name ASC";
											$result=mysqli_query($conn,$sql);
										...
										// banner rotating section
											...
											$sql22 = "SELECT * FROM `brands_irri` WHERE `i_id` LIKE '%i%' ORDER BY name ASC";
										...
										// banner rotating section
											...
											while($row = mysqli_fetch_array($result)) {
																...
																if ($xxx == 0) {
																	...
																	if (($names[1] == $row['i_id']) || ($names[2] == $row['i_id']) || ($names[4] == $row['i_id']) || ($names[5] == $row['i_id']) || ($names[7] == $row['i_id']) || ($names[8] == $row['i_id']) || ($names[10] == $row['i_id']) || ($names[11] == $row['i_id']) || ($names[13] == $row['i_id']) || ($names[14] == $row['i_id']) || ($names[16] == $row['i_id']) || ($names[17] == $row['i_id']) || ($names[19] == $row['i_id']) || ($names[20] == $row['i_id']) || ($names[22] == $row['i_id']) || ($names[23] == $row['i_id'])) {$chi2 = 'checked';}else{$chi2='&nbsp';}																	
																	...
																	if (($names[3] == $row['i_id']) || ($names[6] == $row['i_id']) || ($names[9] == $row['i_id']) || ($names[12] == $row['i_id']) || ($names[15] == $row['i_id']) || ($names[18] == $row['i_id']) || ($names[21] == $row['i_id']) || ($names[24] == $row['i_id'])) {$chi3 = 'checked';}else{$chi3='&nbsp';}																	
			...
			?>				
                       ...
                       <input type="submit" value="Submit Irrigation Updates"  style=" background-color: darkgoldenrod; box-shadow: 5px 5px 5px #888888; padding: 3px; font-size: 16px; cursor: pointer" />
			</form>	
		...
		<center><a href='/vendor/index-vendor.php'><img src='https://landscapearchitect.com/imgz2/back-button.jpg' style="box-shadow: 5px 5px 5px #888888" border='0' /></a></center>
