...
function company_results_html($results){
    $html .="<div id='pageBody'>
			<div style='position:absolute; left:25px; top:450px'>
...
//Search Photo by Keyword - @param string $keyword - @return array of photo url start here
function search_photo_key($keyword){
    global $searchby, $vst, $find, $state, $areacodes;
    $kws = explode(" ",$keyword);

    $nlike = each_keyword($kws, 'company_name');
    $nlike2 = each_keyword($kws, 'name');

    //echo "nlike".show_php($nlike);
	$query = "select * from new_vendor where $nlike";
	$query = "select * from xlist where $nlike2";
