...
<h2>Manage Company Product Link</h2>
<hr noshade size='-1' />
<form enctype='multipart/form-data' action='https://landscapearchitect.com/' method='POST' name='fileForm' runat='vdaemon'>
...
<table width='100%' border='0'>
    <tbody>
        <tr>
            <td class='formLabel' width='25%'>Product Name (Read Only)</td>
            <td class='formObject'>
                <input type='text' size='25' name='product_name' id='product_name' value='<?= stripslashes($this->product_name); ?>' readonly />
                <vlvalidator type='required' name='prodReq' control='product_name' errmsg='A product name is required' />
            </td>
        </tr>
        <tr>
            <td class='formLabel' width='25%'>Description (Read Only)</td>
            <td class='formObject'>
                <textarea name='description' rows=10 cols=50 readonly><?= stripslashes($this->description); ?></textarea>
                <vlvalidator type='required' name='descReq' control='description' errmsg='A description is required' />
            </td>
        </tr>
        <tr>
            <td class='formLabel' width='25%'>Web Landing Page (Edit)</td>
            <td class='formObject'>
        ...
        <tr>
            <td class='formLabel' width='25%'>Photos (Read Only)</td>
            <td class='formObject'>
            <? if(isset($this->id)) { ?>
                <? if(isset($this->photo) && strlen($this->photo) > 0) { ?>
                    <input type='radio' name='useold' checked value='1' /> Assigned Photo <br />
    ...
    <tfoot>
        <tr>
            <td align='left'><a href='https://landscapearchitect.com/vendor/index-vendor-auto?action=edit&pass=<? echo $pass; ?>'><img src='/imgz/vendor/exit.gif' border='0' /></a></td>
...
function action(){
	...
	elseif(isset($_GET['action']) && $_GET['action']=='mark'){
	    ...
	    header("Location: photo_edit-auto.php?company_id=$company_id");
	}
