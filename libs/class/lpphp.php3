...
class lpphp
  ...
  function lpphp($lpapi)
    {
    //suppress almost all errors & warnings
    #$oldv=error_reporting(16);
    $this->crlf="\r\n";
  //  $this->lpapi="../cgi-bin/lbin";		// enter path to your lbin here
  //$this->lpapi=getenv( "COMSPEC" ) . " /C D:\\Data\\Web\\Staging\landscapeOnline\\web\\bin\\linkpoint\\lbin.exe";	// Win32	
  // $this->lpapi="C:/WINNT/system32/netsrv/php/linkpoint/";	
  //$this->lpapi="lbin.exe";	// Win32	
  $this->lpapi=$lpapi;
	}
...
###########
  ...
  function process($pdata,$mycf)
    ...
    if($retval!=0)
      {
      $retarr["approved"]="ERROR";
      $retarr["error"]="Error executing LBIN!<br>\n";
      return $retarr;
      }
...
function BillOrders ($myarg)
  ...
  while ($idx < $count)
	...
	$myarg["orders"][$idx]["result"] = $myarg["result"];
       ...
       $ret= $this->BillOrder(&$myarg["orders"][$idx]);
