...
class vendor extends template_class {
    ...
    function displayListingSummary($banner=TRUE) {
     ...
     function vendor_prod($id=NULL, $admin=NULL) {
		...
		// width used to control text from running under banners
        echo("<table style='table-layout: fixed; width:763px'>");
        if(count($prodList) == 0) {
            ...
            foreach($prodList as $row) {
                ...
                <tr>
                    <td valign='top'>
                        ...
                        <a href="<?= stripslashes($row['web_photo']); ?>" target="_blank"> <img src='/products/images/<?= $row['photo'] ?>' <?= $size ?> /> </a> 
                    ...
                    <td width='400' align='left'>
                        ...
                        <b><?= iconv('CP1252', 'ASCII//TRANSLIT', (stripslashes($row['product_name']))); ?></b><br />
                        <?= iconv('CP1252', 'ASCII//TRANSLIT', (stripslashes($row['description']))); ?><br /><br />
                        
                        <? $str = preg_replace('#^https?://#', '', $row['web']); ?>
                                            
                        <strong>Product Link:</strong><br /><a href="http://<?= stripslashes($str); ?>" target="_blank"><?= stripslashes($row['web']); ?></a><br />                    

                        </div>
                    </td>
                </tr>
                <tr><td colspan='2'><div style='width:750px; height:2px; background-color:#605b51;'></td></tr>
                <?
            }
        }
        echo("</table><br>");
    ...
    function getConsite() { return($this->contact_us); }
	...
	// This is the selection for the business type for company info
	function vendorTypeWidget() {
        $vt = new vendor_type();
		
		$venNum = $this->vendor_type_id;
		
		
		if (($venNum == 4) || ($venNum == 6)) {
		
        	$vt->selectionWidget($this->vendor_type_id);
			
		} elseif (($venNum == 1) || ($venNum == 2)) {
			
			$vt->selectionWidget2($this->vendor_type_id);			
			
		} else {
			
			$vt->selectionWidget3($this->vendor_type_id);			
     ...
     ************************************************************************/
     function search_list($find, $xlist="", $filter="", $type = null) {
        $xl = new xlist();
        ...
        if($count = count($children)) {
            ...
            ?>
				<? $nextx = $_GET[next]; ?>
					...
					<? if ($nextx == '38') { ?>
						 ...
						 <table><tr><td colspan="3"><a href="https://landscapearchitect.com/products/index-product.php?localtype=manu_imp&vst=&next=1211&ca=no&searchby=nl&state=&searchtype=photo">	* For Tools See Equipment Tools</a></td></tr></table>
            ...
            <?
                ...
                foreach($children as $idx => $obj) {
                    ...
                    ?>
                    <tr>
                      <td><input <?= $t ?> type='checkbox' name='find[]'  value='<?= $obj['id'] ?>' class="check<?php echo $obj['id'] ?> allcheck" style="display: none"/>
                        </td>
                        <?php if( isset($type) && !empty($subcat)) { ?>
                        <td style="font-size:14px"> <?= $obj['name'] ?></td>
                        <?php } else { ?>
                         <td style="font-size:14px"> 
			<?php /* <input type='text' name='correct_val_' class="correct_val" value='<?php echo  $obj['id'] ?>' /> */ ?>
            
      
				
						<a href="javascript:void(0)" class="clicktosearch1" onclick="return clicktosearchfun('<?php echo  $obj['id'] ?>');" >
            
                    
                         <?= $obj['name'] ?>  
                         <?php 
                            echo $this->get_product_count($obj['id']); ?> </td>
                         <?php } ?>
                         </a>
    ...
    function view_wh() {
     ...
     function get_product_count( $category_id = null){
        $num_rows = '';
        $select_query = "SELECT * FROM  `vendor_product` WHERE FIND_IN_SET( $category_id, `xlist` ) >0 ";
    ...
    function vendor_xlist_catp_find2($find) {
        $xl = new xlist();
        $list = $xl->fetchXlistWholesaleParentArray2();
        foreach($list as $obj) {
            $t = ($obj['id'] == $find) ? "selected" : "";
            if($obj['idAccount'] == "0") {
             ?>
	    ...
	    function vendor_xlist_catp_find10($find) {
        $xl = new xlist();
        $list = $xl->fetchXlistParentArray10();
        foreach($list as $obj) {
            $t = ($obj['id'] == $find) ? "selected" : "";
            if($obj['idAccount'] == "0") {
             ?>
    ...
    function vendor_type_list($search=0) {
        ...
        if(is_array($search)) {                                                             
            ...
            //} else {                                                                      
                ...
                $query .= "WHERE `id` != 11 " . implode(" OR ",$search) . " ";                         
                $query .= "ORDER BY id";                                                    
            //}                                                                             
            ...
            $query = "SELECT * FROM vendor_type WHERE `id` != 11 ORDER BY id";                               
        }                                                                                   
