	...
	$(document).ready(function() {
		$('#keyword').on('input', function() {
			...
			if (searchKeyword.length >= 2) {
				$.post('search-test.php', { keywords: searchKeyword }, function(data) {
					$('ul#content').empty()
					$.each(data, function() {
						$('ul#content').append('<li><a href="https://landscapearchitect.com/industry/index-result.php?uid=0&type_event=meeting&action=search&search=acronym&acro=' + this.id + '" target="_blank">' + this.title2 + '</a></li>');
					});
				}, "json");
			}
		});
