...
<div>
	...
	<div style="position:relative; left: 10px; top: 0px; z-index: 0">
				...
				<div style="position:absolute; left: 525px; top:255px; z-index: 0">
						...
						<form method="post" action="prod-seac-all-prod.php">
	...
	<div style="position:relative; left: 10px; top: 0px; z-index: 0">
                   ...
                   <h3 style="font-family: Helvetica, Arial,' sans-serif'; font-size: 18px; padding-left: 330px">Results:</h3><br>
						...
						$sql = 	"(SELECT id, product_name, description, photo, photo_time as 'date', 'product' As type FROM `vendor_product` WHERE `product_name` LIKE '%" . $keywordSE2 . "%' OR `description` LIKE '%" . $keywordSE2 . "%')
							UNION
						...
						while ($list = mysql_fetch_assoc($result)) {
						   ...
						   echo "<h4>View More News for: " . $keywordSE . "</h4>";
						...
						for ($x = ($currentpage - $range); $x < (($currentpage + $range) + 1); $x++) {
						   ...
						   if (($x > 0) && ($x <= $totalpages)) {
							  ...
							  // if not current page...
								 ...
								 // make it a link
								 echo " <a href='{$_SERVER['PHP_SELF']}?currentpage=" . $x . "&keywordSE3=" . $keywordSE ."'>$x</a> ";
							  } // end else
						   } // end if 
						} // end for
						...
						if ($currentpage != $totalpages) {
						   ...
						   echo " <a href='{$_SERVER['PHP_SELF']}?currentpage=" . $nextpage . "&keywordSE3=" . $keywordSE ."'>></a> ";
