...
$results = mysqli_query($connecDB,"select * from vendor_product LEFT JOIN new_vendor ON new_vendor.id=vendor_product.vendor_id  LEFT JOIN xlist ON xlist.id=vendor_product.xlist  where new_vendor.co_name2 like '%" . $key . "%' ORDER by new_vendor.status DESC, vendor_product.photo_time DESC LIMIT $position, $item_per_page");
