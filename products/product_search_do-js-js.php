...
if($searchtype == 'photo' && $proceed == true){
  ...
  if(isset($keyword) && $keyword!='') {
    ...
    if(!$results) {
        echo "<div id='pageBody'>
        <center>
      <div style='position:absolute; left:25px; top:480px'>
          <table width='753' border='0' align='center'>
        <tr>
          <td style='font-size:15px; color:red'>No Matches Found. Please Try Again.</td>
        </tr>
...
function company_results_html($results){
    ...
    $html .=" $logo";
      ...
      if($r['status'] >= 4) $html .=" <a href='listing.php?id=$r[id]' target='_blank'>";
      $html .="<span style='font-size:14px; font-weight:200'> $r[company_name] </span>";

      if($r['status'] >= 4) $html .=" </a>";
      $html .=" <br><span style='font-size:14px; font-weight:200'>";
      $html .= $r['phone'];
      $html .= '</span><br><br>';
      $html .= "</td>";
...
function photo_results_html($results){
										...
										// The loop divides the photos between status 16 and 2
										// products
										...
										// start for the products add counting and getting from table
											$arry = $_GET[find];
											$catf =  $arry[0];

											$sql = "SELECT * FROM vendor_product LEFT JOIN new_vendor ON vendor_product.vendor_id=new_vendor.id WHERE vendor_product.xlist = '" . $catf ."' ORDER BY vendor_product.photo_time DESC";
										...
										// Product status 16
											...
											echo '<div style="font-size: 14px; font-weight: bold;"><div style="margin-top: 60px; margin-left: -15px;"><center><span style="font-size:28px; font-weight:bold">Certified Vendors</span></center><br><table width="763">';	
											...
											while($row = mysqli_fetch_array($result)) {
												...
												echo "<td align='left'><div style='overflow: hidden; padding: 10px; color:#906; background:#CCC; border-bottom:#F0F0F0 1px solid'><a href='listing.php?id=" . $row[id] . "' target='_blank'><img src='https://landscapearchitect.com/products/images/".$row[photo]."' border='0' width='233px'></div><span style='color:#906'>" . $row[company_name] . "</span></a><br />" . iconv('CP1252', 'ASCII//TRANSLIT', (($row[product_name]))) . "</td>";
											...
											echo "</table></div></div><br><div style='width:750px; height:2px; background-color:#605b51;'> </div><br>";
											
								
										// Product status 2
										
											echo "<center><span style='font-size:28px; font-weight:bold'>Archived Listings</span><br />
											<span style='font-size:8px'>The information in the listings below may or may not be current</span></center>";
											$arry = $_GET[find];
											$catf =  $arry[0];
										...
										// Product status 2
											...
											echo '<div style="font-size: 14px; font-weight: bold;"><div style="margin-top: 60px; margin-left: -15px;"><table width="763">';	
