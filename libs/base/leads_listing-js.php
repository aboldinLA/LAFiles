...
class vendor extends template_class {
    ...
    function vendor_prod($vendor_id=NULL, $admin=NULL) {
        $vp = new vendor_product();
		

        if(is_null($vendor_id) || !is_numeric($vendor_id)) {
            $vendor_id = $this->id;
        }
        ...
        $prodList = $vp->fetchVendorArray($vendor_id);
    ...
    function slurpCompanyForm() {
       ...
       if(is_null($_REQUEST['id'])) {
            $query = "INSERT INTO new_vendor (id, vendor_type_id, active, status, company_name, website, logo, info_request, sentpass, input_date,".
            "edit_date, profile, hq, web_mod, pass, email, address, city, state, zip, area_code, phone, fax) VALUES ".
            "(NULL, '$vendor_type_id', '$active', '$status', '$company_name', '$website', '$logo', '$info_request', '$sentpass', NULL, ".
            "NULL, '$profile', '$hq',1 , '$pass', '$email', '$address', '$city','$state', '$zip', '$area_code', '$phone', '$fax')";
        } else {
            $uid = $_REQUEST['id'];
            $query = "UPDATE new_vendor SET vendor_type_id = '$vendor_type_id', company_name = '$company_name', website = '$website', logo = '$logo',".
