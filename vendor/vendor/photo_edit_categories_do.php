...
function action(){
    ...
    header("Location: photo_edit.php?company_id=$_POST[company_id]");
}
...
function get_children_html($product_id){
    ...
    foreach($parents as $key=>$parent){
        $html .= "<tr><td class='cellhead' colspan='2'><a name='id$parent[id]'><h3>$parent[name]</h3></a></td></tr>";
        ...
        foreach($children[0] as $k=>$child){
            $checked1 = ""; $checked2 = "";
            if(isCurrentlySelected($product_id, $child[id])){
                $checked1 = "checked";
                $count++;
            }
            if(isCurrentlySelected($product_id, $children[1][$k]['id'])){
                $checked2 = "checked";
                $count++;
            }
        ...
        $html .= "<tr><td colspan='2'><a href='#topList'><h4>Back to Top</h4></a></td></tr>";
...
function getChildren($parent_id){
    $query = "select * from xlist where idParent=$parent_id";
    $result = query($query);
    return chunk($result);
}
