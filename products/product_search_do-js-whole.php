...
if($searchtype == 'company' && $proceed == true){
	...
	if(isset($keyword) && $keyword!=''){
   		$results = search_company_key2($keyword);
	}
    else{
    	$results = search_company2($next);
    }    	
    if(!$results){
        echo "<div id='pageBody'>
        <center>
			<div style='position:absolute; left:543px; top:160px'>
        	<table width='753' border='0' align='center'>
				<tr>
					<td style='font-size:15px; color:red'>Please Choose a State.</td>
				</tr>
			</table>
			</div>
            </center>
        ...
        echo company_results_html2($results);
}

if($searchtype == 'photo' && $proceed == true){
    echo add_tabs2();
	if(isset($keyword) && $keyword!='') {
   		$results = search_photo_key2($keyword);
    } else
    	$results = search_photo2($next);
    if(!$results) {
        echo "<div id='pageBody'>
        <center>
			<div style='position:absolute; left:25px; top:285px'>
        	<table width='753' border='0' align='center'>
				<tr>
					<td style='font-size:15px; color:red'>No Matches Found. Please Try Again.</td>
				</tr>
			</table>
			</div>
            </center>
        ...
        echo photo_results_html2($results);
}

function company_results_html2($results){
    $html .="<div id='pageBody'>
			<div style='position:absolute; left:25px; top:475px'>
        	<table width='753' border='0'><tr>
            	    <td style='font-size:16px'>Company</td>
            	    <td align='center'> </td>
            	    <td width='20'>&nbsp;</td>
            	    <td style='font-size:16px' width='226'>Company</td>
            	    <td align='center'> </td>
            	</tr>";
    $counter=0;
    foreach($results as $r){
	    ...
	    if($r['status'] >= 4) $html .="	<a href='listing.php?id=$r[id]' target='_blank'>";
...
function photo_results_html2($results){
    ...
    foreach($results as $r){
        ...
        if($counter % 3 == 0)
            $html .= "<tr>";
        $html .= "<td align=left><div style='overflow: hidden; padding: 10px; color:#906; background:#CCC; border-bottom:#F0F0F0 1px solid'>";
        if($r['status'] >= 4) $html .= "<a href='listing.php?id=$r[vendor_id]' target='_blank'>";
        $html .= "<img src='images/$r[photo]' border='0'width='233px'></div><br />$r[product_name]<br /><span style='color:#906'>$r[company_name]</span>";
...
function search_company2($next){
    global $searchby, $vst, $find, $state, $areacodes;
    $extra = (isset($where) || isset ($clike))? " and ($where $clike)" : "";
    $query2 = find_listings2($searchby, $extra, $vst, $find, $state, $areacodes);
...
function search_company_key2($keyword){
    global $searchby, $vst, $find, $state, $areacodes;
    
    $kws = explode(" ",$keyword);
    $nlike=each_keyword2($kws, 'name');
    $clike=each_keyword2($kws, 'company_name');
    ...
    if($result_by_xname){
        $where = xlistor2($result_by_xname, 'v');
        $where .= " OR ";
    }

    $query2 = find_listings_key2($searchby, "and ($where $clike)", $vst, $find, $state, $areacodes);
    $result_all_vendors_by_xlist = query($query2);
    if(count($result_all_vendors_by_xlist)==0)
        return false;
...
function search_photo_key2($keyword){
    global $searchby, $vst, $find, $state, $areacodes;
    $kws = explode(" ",$keyword);
    ...
    $clike=each_keyword2($kws, 'v.company_name');
    ...
    $where = xlistor2($result_by_xname, 'vp');
    if($where != "") $where .= ' OR '; 
...
function find_listings_key2($searchby, $extra, $vst, $find, $state, $areacodes) {
...
function find_listings2($searchby, $extra, $vst, $find, $state, $areacodes) {
     $query = "SELECT v.* FROM  new_vendor as v WHERE v.active=1 ";
    if($searchby == "ac") { 
    	if($areacodes != ""){
    		$ary_areacodes = explode(" ", trim($areacodes));
...
function xlistor2($result_by_xname, $table=''){
    $where="";
    $count=0;
...
function each_keyword2($kws, $table){
    $like="(";
    $counter=0;
    foreach($kws as $kw){
        if($counter != 0)
...
<div style="position:absolute; left:5px; top:395px; padding:0px; width:665px; font-size:14px; color:#000">
<br />
<!--To refine your search, please click on the tabs below.--><br />
<table border='0' cellpadding='0' cellspacing='0' width='753'>
    <tr>
        <td width="95" align="center" nowrap style="height:30px; font-family:Arial, Helvetica, sans-serif; font-size:16px; font-weight:bold; color:#000; background-color:#a97f3f; padding-left:10px; box-shadow: 5px 5px 5px #888888"><strong>Search Results</strong></td>
