...
if($REQUEST_METHOD=="POST")	{
	...
	if(!strlen($error))	 {  
		...
		if ($subscribe == "LASN") {$listing = "1";}

		if ($subscribe == "LCM") {$listing = "2";}
		...
		if ($action == "renew" ||  $action == "edit" ) {
			...
			$M->subscribtion_form_edit($uid, $comp_name, $sal, $first_name, 
			...
			if ($subscribe == 'LASN')
				...
				header("location: design.php?action=".$action."&media_id=$uid");

			 

			if ($subscribe == 'LCM')

				header("location: contractor.php?action=".$action."&uid=$uid");     
			...
			if (is_numeric($total)) { 
				...
				$uid = $M->subscribtion_form($comp_name, $sal, $first_name, $last_name, 

						$address, $address_2, $city, $state, $zip, $country,$mail_to,$alt,

						$website,$area_code, $phone, $area_code_fax, $fax,

						$email,$month, $how, $how_other, $subscribe, $note,$listing,0);
