...
$results = mysqli_query($connecDB,"select * from vendor_product LEFT JOIN new_vendor ON new_vendor.id=vendor_product.vendor_id LEFT JOIN xlist ON xlist.id=vendor_product.xlist where (xlist.name like '%" . $key . "%'AND xlist.id > '99') ORDER by vendor_product.photo_time DESC LIMIT $position, $item_per_page");
