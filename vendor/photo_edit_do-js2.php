...
function action(){
	...
	elseif(isset($_GET['action']) && $_GET['action']=='mark'){
	    ...
	    header("Location: photo_edit-js.php?company_id=$company_id");
	}
...
function get_products_list_html($company_id){
    ...
    foreach($products_list as $product){
        ...
        $html .="<tr>";
        $html .="<td align='left' width='60%'><h3>Product Name: </h3>".iconv('CP1252', 'ASCII//TRANSLIT', (stripslashes($product[product_name])))."";
    ...
    $pattern2='â€™';
    $repl2='&#39;';
    $demogs2 = iconv('CP1252', 'ASCII//TRANSLIT', (stripslashes(str_replace($pattern2, $repl2, $product[description]))));
	
			

        $html .="<br><br><h3>Description: </h3>$demogs2<br>";
		
 		$dog = $product[web];
		$html .='<br><strong>Web Link:</strong><br><span style="font-size: 14px">(This should link to the Specific Product Page and NOT just your main Web Address.)</span><br><a href="' . $dog . '"><strong>' .  $product[web] .  '</strong></a>' .  '<br>';			
		
        $html .=get_current_category_selected($product['id'],$company_id)."<br>";
		

		$html .='</td>';	
		
		// 		$html .='<br><strong>Web Link:</strong><br><a href="' . $dog . '">' .  $product[web] .  '</a>' .  '<a href="/private/vendor/index2.php?action=products&intent=edit&record=' . $company_id .  '&pid=' . $product[id] . '">' . '&nbsp;&nbsp;(edit link)<br>' . '</a>' . '</td>';		

				
		
		
        $html .="<td>";
        $html .="<img width='225' src='/products/images/$product[photo]' align='right' style='box-shadow: 5px 5px 5px #888888'>";
        $html .="</td>";
        $html .="</tr>";
        $html .="<tr>";
        ...
        $html .="<br><img src='/imgz/vendor/pencil.gif' onclick='window.location=\"/vendor/profile-js.php?action=products&intent=edit&record=$company_id&pid=$product[id]\"'> &nbsp;";
        $html .="<img src='/imgz2/delete3.jpg' style='box-shadow: 5px 5px 5px #888888' onclick='window.location=\"/vendor/photo_edit.php?action=mark&company_id=$company_id&product_id=$product[id]\"'>";
        //$html .="<br><a href = 'photo_edit_categories.php?product_id=$product[id]&company_id=$company_id'>Edit Categories </a>&nbsp;";
        //$html .="<img src='/imgz/vendor/delete.gif' onclick='window.location=\"/private/vendor/photo_edit_do.php?action=mark&company_id=$company_id&product_id=$product[id]\"'> &nbsp;";
        $html .="</td>";
        $html .="</tr>";
        $html .="<tr>";
        $html .="<td colspan='3' style='height:20px'><div style='width:775px; height:2px; background-color:#605b51;'> </div><br>";
        $html .="</td>";
        $html .="</tr>";		
        $counter=1;
    }
...
function get_products($vendor_id){
    $query = "select * from vendor_product where vendor_id = $vendor_id order by photo_time DESC";
    return query($query);
}
...
function delete_product_photo($product_id, $company_id){
    ...
    $message .= "<img width='225' src='https://landscapearchitect.com/products/images/".$info[0]['photo']."'> <br>";
